...
export function createOceanSky(scene: THREE.Scene): (t: number) => void {
  ...
  return (t: number) => {
    ...
    bg.addColorStop(0.0, '#c8f0ff');
    bg.addColorStop(0.12, '#5bbde0');
    bg.addColorStop(0.35, '#1a7ab8');
    bg.addColorStop(0.6, '#0a4a78');
    bg.addColorStop(0.8, '#052a50');
    bg.addColorStop(1.0, '#010e1e');
    ctx.fillStyle = bg;
    ...
    for (let i = 0; i < RAY_COUNT; i++) {
      ...
      const wobble = Math.sin(t * 0.22 + i * 0.9) * 0.06 + Math.sin(t * 0.11 + i * 1.7) * 0.03;
    ...
    for (let i = 0; i < 22; i++) {
      ...
      const ca = 0.1 + Math.sin(t * 0.4 + i * 1.05) * 0.06;
