...
export function createCausticOverlay(scene: THREE.Scene, floor: FloorSampler): (t: number) => void {
  ...
  return (t: number) => {
    ...
    for (let i = 0; i < 38; i++) {
      ...
      const br = size * 0.04 + Math.sin(t * 0.55 + i * 0.62) * size * 0.022;
      const a = 0.3 + Math.sin(t * 0.45 + i * 1.1) * 0.18;
...
export function createWaterPuddles(scene: THREE.Scene, floor: FloorSampler): (t: number) => void {
  const spots = [
    { x: 0.5, z: 1.4, rx: 0.75, rz: 0.5, seed: 1.3 },
    { x: 1.5, z: -3.0, rx: 0.6, rz: 0.44, seed: 2.7 },
    { x: -2.8, z: 3.8, rx: 0.62, rz: 0.46, seed: 0.8 },
    { x: 0.0, z: -0.5, rx: 0.68, rz: 0.5, seed: 4.1 },
  ...
  for (const sp of spots) {
    ...
    ticks.push((t: number) => {
      ...
      bg.addColorStop(0.8, 'rgba( 5, 32,  90, 0.48)');
      ...
      for (let i = 0; i < 6; i++) {
        ...
        const pr = R * (0.1 + 0.05 * Math.sin(t * 0.35 + i * 0.9));
      ...
      const hx = cx - R * 0.22,
        hy = cy - R * 0.26;
      const hl = ctx.createRadialGradient(hx, hy, 0, hx, hy, R * 0.3);
  ...
  return (t: number) => {
    ...
    for (let b = 0; b < BANDS; b++) {
      ...
      for (let x = 0; x <= SZ; x += 2) {
        const y =
          yBase +
          Math.sin((x / SZ) * Math.PI * 2 * freq + t * spd + b * 1.3) * amp +
...
export function createSandDetail(
  scene: THREE.Scene,
  floor: FloorSampler,
  sandDiff: THREE.Texture,
): void {
  ...
  for (let py = 0; py < SZ; py++) {
    for (let px = 0; px < SZ; px++) {
      ...
      const n =
        0.5 +
        0.22 * Math.sin(nx * 0.68 + nz * 0.42 + 1.1) +
        0.14 * Math.sin(nx * 1.55 + nz * 1.08 + 2.45) +
        0.08 * Math.cos(nx * 2.9 + nz * 0.75 + 0.65) +
        0.05 * Math.sin(nx * 0.32 + nz * 2.1 + 3.2);
